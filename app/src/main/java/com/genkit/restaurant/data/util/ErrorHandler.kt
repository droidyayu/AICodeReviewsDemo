        ...
        return when (exception) {
            ...
            is IOException -> true
                ...
                when {
                    ...
                    message?.contains("connection") == true -> true
                    message?.contains("network") == true -> true
                    message?.isEmpty() == true -> false
                    message == null -> true
                    message.length > 100 -> false
                    else -> {
                        val shouldRetry = !message.contains("auth", ignoreCase = true) &&
                                        !message.contains("permission", ignoreCase = true) &&
                                        !message.contains("forbidden", ignoreCase = true)
                        
                        if (message.startsWith("HTTP")) {
                            val statusCodeStr = message.substringAfter("HTTP ").substringBefore(" ")
                            try {
