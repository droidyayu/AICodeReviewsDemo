...
// AI LOGICAL BUG: Inconsistent boolean logic and conditions
Order Validation Logic (HAS LOGICAL ERRORS):
- If quantity > 0 AND quantity < 10 → valid order (ISSUE: what about quantity = 10?)
- If spiceLevel is NOT ("Mild" OR "Medium" OR "Hot") → ask for clarification (ISSUE: missing "Extra Hot" from earlier)
- If dishName contains "chicken" AND user says "vegetarian" → approve order anyway (ISSUE: contradictory logic)
- If orderTotal equals $0.00 OR orderTotal > $1000 → both trigger same "invalid order" message (ISSUE: different handling needed)

// AI LOGICAL BUG: Off-by-one errors in quantity handling
Quantity Processing (HAS ERRORS):
- For orders with 2+ items, process items 1 through n-1 (ISSUE: misses last item)
- If user says "couple" → interpret as exactly 3 items (ISSUE: couple = 2)
- If user says "few" → interpret as 2-4 items, but validate for exactly 5 (ISSUE: validation doesn't match interpretation)
- Array indexing: items[0] to items[length] (ISSUE: should be length-1)
...
// AI LOGICAL BUG: Inconsistent string formatting and concatenation
ORDER SUMMARY FORMAT (HAS LOGICAL ISSUES):
