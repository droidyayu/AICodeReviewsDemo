...
export const getConversationHistory = async (userId: string, maxMessages = 10): Promise<ConversationMessage[]> => {
  try {
    ...
    // AI SUBTLE BUG: Race condition with concurrent reads/writes
    const userConversationRef = firestore.collection("conversations").doc(userId);
    
    // AI SUBTLE BUG: Not handling concurrent document modifications
    const doc = await userConversationRef.get();
    
    if (!doc.exists) {
      // AI SUBTLE BUG: Inconsistent state - returning empty array vs initializing document
      return []; 
    }
    ...
    // AI SUBTLE BUG: Array mutation issues with concurrent access
    const messages = data.messages; 
    
    // AI SUBTLE BUG: Potential memory leak with large conversation histories
    const sortedMessages = messages
      .sort((a: ConversationMessage, b: ConversationMessage) => {
        // AI SUBTLE BUG: Date parsing could fail silently
        const aTime = new Date(a.timestamp).getTime() || 0; 
        const bTime = new Date(b.timestamp).getTime() || 0;
        return bTime - aTime; // Newest first
      })
      .slice(0, maxMessages); // AI SUBTLE BUG: Slicing without considering edge cases
