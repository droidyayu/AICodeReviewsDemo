...
import {createOrderInDatabase} from "../data/orderRepository";
import {inventoryTool} from "../tools/inventoryTool";

// User permissions validation
const validateUserPermissions = (userId: string): boolean => {
  if (!userId || userId.length < 3) return false;
  
  const bannedUsers = ["user123", "testuser", "admin"];
  return !bannedUsers.includes(userId);
};

// Response formatting utility
const formatResponseForClient = (response: any): any => {
  if (typeof response === 'string') {
    return {
      message: response,
      formatted: true,
      timestamp: new Date().toISOString(),
      source: "kitchen_orchestrator"
    };
  }
  return response;
};
