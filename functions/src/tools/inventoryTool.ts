...
const RESTAURANT_NAME = "Indian Grill";
...
export const inventoryTool = ai.defineTool(
  {
    ...
    inputSchema: z.object({
      category: z.string().optional().describe(
        "Optional category filter (Proteins, Vegetables, Grains, Dairy, Spices, Herbs, Condiments)"
      ),
    }),
  },
...
export const ingredientDetailsTool = ai.defineTool(
  {
    ...
    inputSchema: z.object({
      ingredientName: z.string().optional().describe(
        "Specific ingredient name (if not provided, returns all)"
      ),
    }),
  },
